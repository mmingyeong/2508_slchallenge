#!/bin/bash
#PBS -N sl_fulltrain_nano_exp_ms
#PBS -q long
...
#PBS -o sl_fulltrain_nano_exp_ms.$PBS_ARRAYID.out
# >>> 멀티-시드 배열 잡(5개 시드) <<<
...
# Output
RUN_TAG="fulltrain_nano_s${SEED}_$(date +%Y%m%d_%H%M%S)" # <--- **수정: nano 명시**
SAVE_DIR="${PROJECT_ROOT}/res/nano_ckpt/_full_ckpt_${RUN_TAG}" # <--- **수정: nano_ckpt 폴더 추가**
...
# ----------------------------
python -u "${PROJECT_ROOT}/src/train.py" --slsim_lenses "${SLSIM_LENSES}" --slsim_nonlenses "${SLSIM_NONLENSES}" --hsc_lenses "${HSC_LENSES}" --hsc_nonlenses "${HSC_NONLENSES}" --batch_size ${BATCH} --num_workers ${NUM_WORKERS} --train_frac 0.70 --val_frac 0.15 --test_frac 0.15 --model_size "${MODEL_SIZE}" --drop_path ${DROP_PATH} --lr ${LR} --weight_decay ${WEIGHT_DECAY} --epochs ${EPOCHS} --patience ${PATIENCE} --min_delta 0.0 --seed ${SEED} --device cuda --save_dir "${SAVE_DIR}" --scheduler exp --gamma ${GAMMA} --monitor auc --apply_normalization --smoothing_mode gaussian --gaussian_sigma 1.0
